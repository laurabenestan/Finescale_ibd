  ...
  pblapply(1:repboot, function(j){
    ...
    corrGeoGen <- do.call(rbind, lapply(window,function(i) { 
      ...
      corr_GeoGen_random <- do.call(rbind, mclapply(1:repnull,function(k) {
        ...
        dataGen_boot_random$Ajk <- sample(dataGen$Ajk,min)
        lm_random <- lm(dataGen_boot_random$Ajk~dataGen_boot_random$DIST)
      ...
      lm_boot <- lm(dataGen_boot$Ajk~dataGen_boot$DIST) #
